...
public class Ukutils {
    ...
    /**
     * Registers a keybinding with an associated action, executed when the key is pressed.
     *
     * @param keyMapping The keybinding
     * @param action     The action to be performed when the key is pressed
     */
    public static void registerKeybinding(KeyMapping keyMapping, Consumer<Minecraft> action) {
        PlatformUkutils.INSTANCE.registerKeyBindingInternal(keyMapping);
        keybindings.put(keyMapping, action);
    }
