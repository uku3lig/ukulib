...
public class BrokenConfigScreen extends CloseableScreen {
    ...
    public BrokenConfigScreen(Screen parent) {
        super(Component.nullToEmpty("Broken config screen"), parent);
    }
    ...
    protected void init() {
        ...
        body.addChild(new StringWidget(Component.nullToEmpty("There was an issue with this config screen."), this.font));
        body.addChild(new StringWidget(Component.nullToEmpty("Please report this issue to the mod author."), this.font));

        LinearLayout footer = this.layout.addToFooter(LinearLayout.horizontal().spacing(8));
        footer.addChild(Button.builder(Component.nullToEmpty("Upload logs to mclo.gs"), button -> this.uploadLogs()).build());
        ...
        HTTP_CLIENT.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                .thenAccept(res -> {
                    ...
                    if (apiRes.success && apiRes.url != null) {
                        ...
                        Ukutils.sendToast(Component.nullToEmpty("Error while uploading logs"), Component.nullToEmpty(apiRes.error));
                    }
                });
    }
