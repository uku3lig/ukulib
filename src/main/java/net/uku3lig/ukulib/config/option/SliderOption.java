...
public class SliderOption implements WidgetCreator {
     ...
     */
    public static final DoubleFunction<Component> DEFAULT_VALUE_TO_TEXT = d -> Component.nullToEmpty(String.format("%.2f", d));

    /**
     * Displays a double as a percentage, with no further precision.
     */
    public static final DoubleFunction<Component> PERCENT_VALUE_TO_TEXT = d -> Component.nullToEmpty(String.format("%.0f%%", d * 100));

    /**
     * Displays a double as a rounded integer.
     */
    public static final DoubleFunction<Component> INTEGER_VALUE_TO_TEXT = d -> Component.nullToEmpty(String.format("%.0f", d));
    ...
    private final OptionInstance.TooltipSupplier<Double> tooltipFactory;
     ...
     */
    public SliderOption(String key, double initialValue, DoubleConsumer setter, DoubleFunction<Component> valueToText, double min, double max, double step, OptionInstance.TooltipSupplier<Double> tooltipFactory) {
        this.key = key;
    ...
    public AbstractWidget createWidget(int x, int y, int width, int height) {
        return new DoubleSlider(Component.translatable(key), valueToText, initialValue, min, max, step, setter, tooltipFactory, x, y, width, height);
    }
}
