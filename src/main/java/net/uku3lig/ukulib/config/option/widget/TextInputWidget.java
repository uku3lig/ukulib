...
public class TextInputWidget extends AbstractWidget implements CheckedOption {
    ...
    public TextInputWidget(int x, int y, int width, int height, String initialValue, Consumer<String> changedListener, String suggestion, Predicate<String> textPredicate, int maxLength) {
        super(x, y, width, height, Component.nullToEmpty(suggestion));
        ...
        this.setTooltip(Tooltip.create(Component.nullToEmpty(this.suggestion)));
    ...
    public void renderWidget(GuiGraphics context, int mouseX, int mouseY, float deltaTicks) {
        if (this.isVisible()) {
            if (this.drawsBackground()) {
                ...
                context.blitSprite(RenderPipelines.GUI_TEXTURED, identifier, this.getX(), this.getY(), this.getWidth(), this.getHeight());
            }
            ...
            if (!string.isEmpty()) {
                ...
                context.drawString(this.textRenderer, orderedText, k, this.textY, i, textShadow);
                k += this.textRenderer.width(orderedText) + 1;
            }
            ...
            if (!string.isEmpty() && bl && j < string.length()) {
                context.drawString(this.textRenderer, this.format(string.substring(j)), k, this.textY, i, textShadow);
            }

            boolean canSuggestionBeRendered = this.textRenderer.width(string + suggestion) < this.getInnerWidth();
            if (!this.suggestion.isBlank() && canSuggestionBeRendered) {
                // render the suggestion (if possible)
                int x = this.getX() + this.getWidth() - 4 - this.textRenderer.width(suggestion);
                context.drawString(this.textRenderer, this.suggestion, x, this.textY, CommonColors.GRAY, textShadow);
            }

            if (l != j) {
                int n = this.textX + this.textRenderer.width(string.substring(0, l));
                context.textHighlight(Math.min(m, this.getX() + this.width), this.textY - 1, Math.min(n - 1, this.getX() + this.width), this.textY + 1 + 9, true);
            }
            ...
            if (bl2) {
                if (bl3) {
                    ...
                    context.drawString(this.textRenderer, HORIZONTAL_CURSOR, m, this.textY, i, textShadow);
                }
            }
    ...
    private void updateFirstCharacterIndex(int cursor) {
        if (this.textRenderer != null) {
            ...
            String string = this.textRenderer.plainSubstrByWidth(this.text.substring(this.firstCharacterIndex), i);
            int j = string.length() + this.firstCharacterIndex;
            if (cursor == this.firstCharacterIndex) {
                this.firstCharacterIndex = this.firstCharacterIndex - this.textRenderer.plainSubstrByWidth(this.text, i, true).length();
            }
